		...
		if (!the_queue.empty()) {
			...
			if (plane->ready_takeoff_time == -1) { // new plane has arrived at the front
				...
				if (clock - plane->ready_takeoff_time > departure_time) {
					...
					the_queue.pop(); // removes the plane from the queue
					int time_stamp = departure_time; // creates a time stamp for when the plane leaves
					int wait = clock - time_stamp; // finds the wait time for the plane
					total_wait += wait;  // updates the total wait time
					num_served++;    // updates the number of planes served
