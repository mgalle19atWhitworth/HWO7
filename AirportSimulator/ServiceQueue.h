	...
	void update(int clock) 
	{
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				int wait = time_stamp - clock;
				landing_queue->total_wait += wait;
				landing_queue->num_served++;
				plane->start_service_time=clock;

				// FIXME: update total_wait and num_served for the landing q
				min_service_time = my_random.next_int(5);
				max_service_time = my_random.next_int(10);
				set_service_times(min_service_time, max_service_time);
